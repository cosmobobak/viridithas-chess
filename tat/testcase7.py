...
data = [d.split(" contain ") for d in data.split("\n")]
colours = {d[0][:-5] : d[1].split(", ") for d in data}

def parents(key):
    return [k for k in colours if any([key in t for t in colours[k]])]

def children(key):
    if colours[key][0] == "no other bags.":
        return {}
    workingdict = {str(i)+d[0]: d[2:] for i, d in enumerate(colours[key])}
    for k, v in workingdict.items():
        if "bags." in v:
            workingdict[k] = v[:-6]
        elif "bag." in v:
            workingdict[k] = v[:-5]
...
def count_parents(node):
    count = 1 if node not in encounteredNodes else 0
    encounteredNodes.add(node)
    for p in parents(node):
        count += count_parents(p)
    return count

callCount = 0
def count_children(node) -> int:
    global callCount
    count = 1
    for num, child in children(node).items():
        for i in range(int(num[1:])):
            count += count_children(child)
            callCount += 1
    return count

print(count_parents("shiny gold")-1)
