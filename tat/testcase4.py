...
data = """iyr:1928 cid:150 pid:476113241 eyr:2039 hcl:a5ac0f
ecl:#25f8d2
byr:2027 hgt:190
...
doublevalids: int = 0
for pp in passports:
    if all([x in pp for x in ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid",]]):
        valids += 1
...
for pp in passports:
    if all([x in pp for x in ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid",]]):
        kvs = [tuple(kv.split(":")) for kv in re.split(" |\n", pp)]
        ppdict = {kv[0]: kv[1] for kv in kvs}
        if not (1919 < int(ppdict["byr"]) < 2003):
            fails[0] += 1
            continue
        if not (2009 < int(ppdict["iyr"]) < 2021):
            fails[1] += 1
            continue
        if not (2019 < int(ppdict["eyr"]) < 2031):
            fails[2] += 1
            continue
        if not (((t := ppdict["hgt"][-2:]) == "cm") or ((t := ppdict["hgt"][-2:]) == "in")):
            fails[3] += 1
            continue
        else:
            num = int(ppdict["hgt"][:-2])
            if t == "cm":
                if not (149 < num < 194):
                    fails[4] += 1
                    continue
                ...
                if not (58 < num < 77):
                    fails[5] += 1
                    continue
        if not (ppdict["hcl"][0] == "#" and all([c in "0123456789abcdef" for c in ppdict["hcl"][1:]])):
            fails[6] += 1
            continue
        if not (ppdict["ecl"] in {"amb", "blu", "brn", "gry", "grn", "hzl", "oth"}):
            fails[7] += 1
            continue
        if not (len(ppdict["pid"]) == 9 and all([c in "0123456789" for c in ppdict["pid"]])):
            fails[8] += 1
            continue
        doublevalids += 1
