...
def swordSearch(node, timeLimit, a, b, colour, endgame):
    startTime = time.time()
    depth = 0
    moves = orderedMoves(node)
    values = []
    ...
    while timeLimit > time.time()-startTime:
        for index, move in enumerate(moves):
            node.push(move)
            values[index] = negamax_killer(node, depth, a, b, colour, endgame)
            node.pop()

        pairs = zip(*sorted(zip(moves, values),key=itemgetter(1)))
        moves, values = [list(pair) for pair in pairs]
        print(values, moves, depth)
        depth+=1
...
def finisher(node, timeLimit, a, b, turn, endgame):
    moves = []
    for move in board.legal_moves:
        moves.append(move)
    moveRatings = []
    for move in moves:
        try:
            ...
            return swordSearch(node, timeLimit, a, b, turn, endgame)
        return moves[moveRatings.index(min(moveRatings))]

#@profile
def pushMove(board,timeLimit,debug):
    start = time.time()
    ...
    try:# (always does a book move if it can)
        ...
        if not endgame:
            best = swordSearch(board, timeLimit, -1337000, 1337000, turn, endgame)
        else:
            best = finisher(board, timeLimit, -1337000, 1337000, turn, endgame)
...
def main(string,rounds,debug,human,side,depth):
    for game in range(rounds):
        ...
        while not board.is_game_over():
            ...
            ratings = pushMove(board,depth,debug)
...
if True:
    ...
    boardArrays,ratingArrays,stack = main(standard,1,True,True,True,8)
...
if False:
    for counter in range(5):
        print(str(counter)+':',swordSearch(chess.Board(standard), counter, -1337000, 1337000, True))
    print('-----------------')
